...
from Protocol.Messages.Server.Battle.BattleEndShowDownMessage import BattleEndShowDownMessage
...
class AskForBattleEndMessage(Reader):
    ...
    def process(self):
        ...
        if self.count == 10:
           BattleEndShowDownMessage(self.client, self.player, self.type, self.result, self.players).send()
        else:
        	pass
